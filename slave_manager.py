...
class SlaveManager(object):
    ...
    def __init__(self, ec2_conn, j, config_file='labels.yml',
                 init_file='slave_init.py', creds=None):
    ...
    def _make_initscript(self, label):
        ...
        manager_auth = b64encode("{0}:{1}"
                                 .format(*self.creds)).replace('\n', '')
    ...
    def start_slaves(self, label, count):
        ...
        reservation = self.ec2_conn.run_instances(self.config[label]['ami'],
                                                  **params)
        for instance in reservation.instances:
            instance.add_tags({'Name': '{0}-jenkins-slave'.format(label)})
        return [self._make_node_name(label, i.id)
...
if __name__ == '__main__':
    ...
    for label, data in state_map.iteritems():
        ...
        idle_nodes = [
            n for n, s in data['nodes'].iteritems()
            if j.get_node_idle_time(n) > manager.config[label]['idle_timeout']
            and not s['pending']
        ]

        if idle_nodes:
            print("{0:<16}: terminating {1} idle node(s):"
                  .format(label, len(idle_nodes)))
