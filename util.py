...
def get_cred_id(self, domain='_'):
    req = Request(self.server + 'credential-store/domain/' +
                  domain + '/api/json')
...
def get_node_idle_time(self, node):
    now = int(time.time())
    groovy = ('script=return Jenkins.instance.getComputer("{0}")'
              '.getIdleStartMilliseconds()').format(node)
...
def get_state_map(self, labels):
    ...
    for label in state_map:
        state_map[label]['jobs'] = queued_jobs.get(label, {}).get('jobs'. {})
    return state_map
